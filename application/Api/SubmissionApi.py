    ...
    def perform_create(self, serializer, format=None):
        ...
        try:
            ...
            if self.request.data.get('input_file') is not None:
                ...
                Output.objects.create(param='log', ext='log', course=challenge.course,
                                      submission=submission, file=myfile)
                ...
                run_eval.delay(result, submission.id)
...
class SubmissionRating(generics.ListAPIView):
    ...
    def get_queryset(self):
        challenge_id = self.request.GET.get('challenge_id')
        input_files = Submission.objects.filter(challenge_id=challenge_id).values("input_file").distinct()
        array = []
        for file in input_files:
            array.append(Submission.objects.filter(input_file=file['input_file'])[0].id)
        return Submission.objects.filter(id__in=array).order_by('-score')
