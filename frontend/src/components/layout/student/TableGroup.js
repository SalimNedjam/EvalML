...
export class TableGroup extends Component {
    ...
    column = [
...

        {
            ...
            key: 'action',
            render: (record) => {
                if (this.props.auth.user.username == record.username)
                    return <a onClick={() => this.doDelete(record)}><FiUserMinus style={IconStyle}/></a>

                if (this.state.user == undefined) {
                    this.setState({
                        user: this.props.listGroup.find(user => this.props.auth.user.username == user.username)
                    })
                }
    ...
    doDelete(record) {
        ...
        confirm({
            ...
            content: record.username + ' ne serra plus dans votre groupe!',
            okText: 'Yes',
            okType: 'danger',
            cancelText: 'No',
            onOk() {
